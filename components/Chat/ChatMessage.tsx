...
export const ChatMessage: FC<Props> = memo(({ message, messageIndex, onEdit }) => {
  ...
  const fileKind =
    message.file?.metadata?.kind ||
    (message.file?.type?.includes('pdf') || fileNameLower.endsWith('.pdf')
      ? 'pdf'
      : fileNameLower.endsWith('.md') || fileNameLower.endsWith('.markdown')
        ? 'markdown'
                      ...
                      <div className="text-sm">
                        ...
                        {message.file.metadata && (
                          <div className="text-xs text-neutral-500 dark:text-neutral-400">
                            {[
                              message.file.metadata.pages
                                ? `${message.file.metadata.pages} pages`
                                : null,
                              message.file.metadata.title || null,
                            ]
                              .filter(Boolean)
                              .join(' - ')}
                          </div>
