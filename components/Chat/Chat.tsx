...
export const Chat = memo(({ stopConversationRef }: Props) => {
  ...
  const buildPromptFromMessages = useCallback((messages: Message[]) => {
    ...
    for (const m of messages) {
      ...
      const attach = m?.file?.metadata?.contentText || m?.file?.metadata?.preview;
      const fname = m?.file?.name;
      const attachLabel = m?.file?.metadata?.contentText ? '' : ' (preview)';
      if (attach && fname && m.role === 'user') {
        lines.push(`[Attachment: ${fname}${attachLabel}]\n${attach}`);
      }
    }
    return lines.join('\n\n');
  }, []);
  ...
  useEffect(() => {
    const modelName = selectedConversation?.model?.name;
    if (!modelName) return;
    if (
      SYSTEM_PROMPT_UNRELIABLE_MODELS.includes(modelName) &&
      warnedModelRef.current !== modelName
    ) {
      warnedModelRef.current = modelName;
      toast.error(
        `Warning: ${modelName} may ignore the System Prompt. Consider switching models for strict behavior.`,
      );
    }
                ...
                {models.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-slide-up">
                    <button
                      ...
                      className="card p-4 text-left hover:shadow-hover transition-all duration-200 hover:scale-[1.02] group"
                    >
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="font-semibold text-gray-900 dark:text-gray-100">RAG Ingestion</h3>
                      </div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Carga tus PDFs, Excel y documentos para que la IA responda con tu información.
                      </p>
                    </button>
                    ...
                    <div className="card p-4">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="font-semibold text-gray-900 dark:text-gray-100">Consejo Pro</h3>
                      </div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        Usa el "Mensaje del sistema" para definir cómo debe comportarse tu asistente IA.
                      </p>
                    </div>
                  </div>
                )}
